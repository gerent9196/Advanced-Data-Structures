...
TestCase * Grader06::testHashMap(std::string filename, std::string iface)
{
  std::vector<HashMapCmd> commands;
  Commands06 cmds06;
  cmds06.loadHashMapCommmands(filename, commands);
  ...
  for(size_t i = 0; i < commands.size(); ++i){
    ...
    if(cmd.action == HASHMAP_CMD_INSERT){
      ...
      if(contains){
        if(cmd.output == COMMANDS06_FALSE){
          return failed("error in containsKey #1");
        }
      } else {
        if(cmd.output == COMMANDS06_TRUE){
...
TestCase * Grader06::testHashMapBulk(std::string data_filename, std::string cmds_filename)
{  
  std::vector<HashMapCmd> commands;
  std::vector<std::pair<CompositeKey, int> > data;  

  Commands06 cmds06;
  cmds06.loadHashMapCommmands(cmds_filename, commands);
  cmds06.loadHashMapData(data_filename, data);
...
std::string Grader06::getDescription(int test_case)
{
  switch(test_case){
...
int Grader06::getPoints(int test_case)
{
  switch(test_case){
  case 0: return 10;
  case 1: return 10;
...
Date Grader06::getDueDate()
{
  return Date(11, 28, 2013);
}
  
TestCase * Grader06::grade(int index){
  switch(index){
  case 0:
