...
private:
  ...
  TestCase * testRTreeHilbert(std::string points_filename, 
    std::string sorted_filename, std::string sorted_filename2);
  TestCase * testRTree(std::string points_filename, std::string cmds_filename);
  ...
  TestCase * checkSort(std::vector<RTreePoint>& harness_sorted, 
