...
TestCase * Grader04::testRTreeHilbert(std::string points_filename, std::string sorted_filename)
{
  ...
  if(points.size() == 0 || gold_sorted.size() == 0){
    return failed("cannot load input file");
  }
...
TestCase * Grader04::testBTree(std::string cmds_filename)
{
  ...
  for(size_t i = 0; i < commands.size(); ++i){
    ...
    if(cmd.action == BTREE_CMD_INSERT){
      ...
      int accepted_height = log4((rb_sorted.size() + 1) / 2) + 1;
