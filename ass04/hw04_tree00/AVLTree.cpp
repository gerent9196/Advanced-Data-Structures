...
void AVLTree::insert(int key){
    ...
    while(temp_root!=NULL){
        cout <<"\nTraversing path: "<< temp_root->getKey();
    ...
    if (path.at(path.size()-1)->getKey() > key){  
        ...
        //add to the right
        cout<<"Adding to right";
        path.at(path.size()-1)->setRight(new_node);
    }
   for(int i=int(path.size())-1; i>=0; i--){
        cout << "\nvalue: "<< path.at(i)->getKey();
        cout << "\tHeight: "<< calcHeight(path.at(i));
        int h = diffHeight(path.at(i));
        path_Bf[i] = h;
        cout << "\t Diff Height: "<< path_Bf.at(i);
        if(h ==2) {
            if(path_Bf.at(i+1) == 1){   
            // 2,1 combination;
                if(i==0){   
...
void AVLTree::remove(int key){
    ...
    if (left_of_to_del->getRight() == NULL){
        while(left_of_to_del != NULL) {
            temp_root->setKey(left_of_to_del->getKey());
            path.push_back(temp_root);
            path_Bf.push_back(0);
            temp_root = left_of_to_del;
            left_of_to_del = temp_root->getLeft();
        }
        delete temp_root;
    }
    else{
